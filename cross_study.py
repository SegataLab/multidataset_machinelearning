...
class cross_study(object):
    def __init__(self, datasets, defined_problem, databases, which_python, n_iter, max_feat, ncores, grid_term0, grid_term1, grid_term2, restrict, how_verbose, in_background):
    ...
    def write_cross_command_lines(self):
        ...
        for db in self.databases:
            datasetcaller = open('../datasetcaller'+(db if isinstance(db, str) else '_'.join(db))+'.sh','w')
    ...
    def output_name(self, db, algo, data, test_i, test):
        ...
        out.append('experimenttype:standard')
        out.append(algo)
    ...
    def svm_experiment(self, db, data, test_i, test):
        dataset = self.dataset_name(data, db, test)
        ...
        line += ['-r 3 -s roc_auc -l svm -d 1:' + ':'.join(test.split(':')[:2])]

        if (not self.utils.isonedata(data)) and (not test_i<0):
            line += ['-t dataset_name:%s' %data[test_i]]
        if isinstance(db, list): 
            line.append('-z '+':'.join([self.utils.features[dab] for dab in db]))
        elif isinstance(db, str): 
            line.append('-z '+self.utils.features[db])
    ...
    def write_cross_study_commands(self, algo):
        ...
        if algo in ['rf','svm']: 
            ...
            algorithm = self.random_forest_experiment if algo=='rf' else self.svm_experiment
        else: 
        ...
        for db in self.databases:
            if not self.restrict == 'lodo':
                ext = open('../transfer_'+(db if isinstance(db, str) else '_'.join(db))+'_'+test+'_'+algo+explanation+'.sh','w')
            ...
            if not self.restrict == 'transfer':
                exl = open('../lodo_'+(db if isinstance(db, str) else '_'.join(db))+'_'+test+'_'+algo+explanation+'.sh','w')
