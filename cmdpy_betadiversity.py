...
class beta_diversity(object):
	...
	if not self.args['load_coordinates'] \
        ...
        print ' YAOOOO'
        print self.sample_and_coordinates, ' sono io sono io sono io'

        if self.args['save_coordinates']: 
            cwd = os.getcwd()
            print 'la current: ', cwd
            self.save_coordinates(self.args['save_coordinates'])

        if self.args['mask']: self.mask()
        if self.args['explain']: self.explain()
        ##print not (not self.args['no_ordination'])
    ...
    def read_params(self, args):
        ...
        arg('-fmt','--format',default='png',type=str)
    ...
    def save_coordinates(self, coordinates_f): 
        print 'sto salvando le coordinate_ '
        self.sample_and_coordinates.to_csv(coordinates_f, sep='\t', header=True, index=True)
