...
class Download(object):
    def __init__(self,bprj,datasetname,aspera,fastqdump,where,vdb\
                ...
                ,dofastA,xml_verbosity,mode="all",samples_to_download=None,split_3=True):
    ...
    def handle_physical_download(self):
        ...
        if not self.mode in ["allready_downloaded"]:  ## onlydownload, all
            for ID in list(all_the_samples):  
                if not os.path.isdir(self.readspath+ID+"/"): os.makedirs(self.readspath+ID+"/") 
                #try:
                self.exec_download_of_any_run(ID)
                #except NameError:
                #    print 'MISSING SAMPLE %s' %ID, 'list of the present keys: ', " ".join(sorted(list(self.idtoruns.keys())))
                n += 1
...
def main(bprj,dataset,asp,fastqdump,downdir,vdb,ncores,dontkeepdownloads,strategy,dontbzip,fasta,xml_verb,modality,stodownload,split_3):
    if not bprj=="NULL": 
        D=Download(bprj,dataset,asp,fastqdump,downdir,vdb,ncores,dontkeepdownloads,strategy,dontbzip,fasta,xml_verb,modality,stodownload,split_3)
    else: 
        ...
        for nm,cd in OPs: D=Download(cd,nm,asp,fastqdump,downdir,vdb,ncores,dontkeepdownloads,strategy,dontbzip,fasta,xml_verb,modality,stodownload,split_3) 
