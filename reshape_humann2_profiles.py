...
def genefamilies(sample,name):
    ...
    s.write(ss[0])
    for a in range(1, len(ss)):
        s.write('\t'.join([ss[a][0], str(ss[a][1])])+'\n') 
    s.close()
...
def read_params():					
   ...
   add('--coverage_f', type=str, default='humann2_PWYcov')
   ...
   add('--coverage_fish', type=str, default='pathcoverage')
...
#        os.mkdir(par['destination_path']+par['dataset']+'/'+par['markerab_f'])
        ...
        #cov_samples = par['origin_path'] +par['dataset']+'/'+ par['coverage_fish'] + '/*.tsv'
