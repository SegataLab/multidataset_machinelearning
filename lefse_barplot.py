...
class effect_size_barplot(object):
    ...
    def read_params(self, args):
        ...
        add('--x_lim', type=int, default=5)
        add('--x_start', type=int, default=2)
    ...
    def plot(self, data, classes):
         ...
         for lab in ax_plot.get_yticklabels():   ##  for d in dir(lab): print d
             ...
             lab.set_size(8 if self.args['number_imp_feat'] < 10 else 4)

         sps = [spine.set_linewidth(1.) for spine in ax_plot.spines.itervalues()]
         #sns.despine(left=False, bottom=False)


         plt.subplots_adjust(right=.6, bottom=0.2)
         plt.savefig(self.args['where'] \
