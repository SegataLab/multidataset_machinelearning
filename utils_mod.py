...
class usefullfuncs(object):
    ...
    def get_std_support_test(self, database, algo, grid0, grid1, grid2, dataset_on_which, training_level):
        return self.res_fold+'plotresult_ANYon_'+dataset_on_which+'_features:'+(database if isinstance(database, str) else '_'.join(database))\
    ...
    def get_all_trans_for_one(self, database, algo, test, grid0, grid1, grid2, dataset_on_which):
        return self.res_fold+'resultoftransfer_*'+'_ON_'+dataset_on_which+'_features:'+('_'.join(database) if isinstance(database, list) else database)\
	      +'_experimenttype:'+test+'_'+algo+'_grid0:'+grid0+'_grid1:'+grid1+'_grid2:'+grid2+'.txt'
...
class effect_size(object):
    ...
    def __init__(self, e, n, c, p):
        self.esize = e
        self.name = n #(n if not isinstance(dictionary, dict) else dictionary[n])
        self.cl = c
        self.pv = p
...
class lefse_reader(object):
    ...
    def select_highest(self, howmany=5):
        ...
        self.feats = feats #if not isinstance(dictionary, dict) else [dictionary[ft] for ft in feats]
        
        #if isinstance(dict, self.dictionary):
        #    if self.with_codes:
        #    else:
        ...
        #print self.feats, ' queste sono le feat'
        #print [d.name for d in self.data[:10]], ' questi i nomi'

        #if self.dictionary:
        #    for dat in self.data:
        #        dat.name = self.dictionary[dat.name]
    ...
    def get(self):
        ...
        #print feat, type(feat), ' questa e la feat'
   	           ...
   	           #     e_size( ( ( feat ) if not isinstance( self.dictionary, dict ) \
                   #     else ( self.dictionary[ feat ] if (not self.with_codes ) else ':'.join( [ feat, self.dictionary[ feat ] ] ) ) ), class_, pvalue ) ) ) 
