...
class training_support(object):
    ...
    def plot_singleton(self, dataset, _values):
        ...
        plt.savefig('%s%s_progressiveLearning_%s_%s.%s' %(self.save_folder, dataset, self.db\
		, 'average' if self.func=='mean' else 'median', self.fig_fmt), dpi=600, edgecolor='black')
